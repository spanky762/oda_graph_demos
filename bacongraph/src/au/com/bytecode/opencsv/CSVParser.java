...
public class CSVParser {
	...
	public CSVParser(final char separator, final char quotechar, final char escape, final boolean strictQuotes,
		...
		if (this.anyCharactersAreTheSame(separator, quotechar,
		        escape)) { throw new UnsupportedOperationException("The separator, quote, and escape characters must be different!"); }
		if (separator == NULL_CHARACTER) { throw new UnsupportedOperationException("The separator character must be defined!"); }
			...
			if (c == this.escape) {
				...
				if (this.isNextCharacterEscapedQuote(nextLine, inQuotes || this.inField, i)) {
					...
					if (!this.strictQuotes) {
						...
						) && (nextLine.length() > (i + 1)) && (nextLine.charAt(i + 1) != this.separator // not at the end of an escape
						                                                                                // sequence
						)) {
