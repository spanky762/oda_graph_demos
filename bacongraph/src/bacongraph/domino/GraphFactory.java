...
public class GraphFactory extends AbstractGraphFactory implements IGraphFactory {
	public final static String GRAPH_NAME = "bacongraph";
	...
	public static class GraphInitializer implements Runnable {
		...
		public void run() {
			...
			ServletUtils.staticConsole(type, method, "starting BaconGraph Graph Initializer");
			try {
	...
	protected FramedTransactionalGraph setupInitialGraph() {
		...
		actor.setStoreKey("demos/bacon/big/actors.nsf");
		actor.addType(bacongraph.model.Actor.class);
		config.addElementStore(actor);

		final DElementStore movie = new org.openntf.domino.graph2.impl.DElementStore();
		movie.setStoreKey("demos/bacon/big/movies.nsf");
		movie.addType(bacongraph.model.Movie.class);
		config.addElementStore(movie);

		final DFramedGraphFactory factory = new DFramedGraphFactory(config);
		result = factory.create(graph);

		// final UserKeyResolver resolver = new UserKeyResolver((DFramedTransactionalGraph) result);
		// graph.addKeyResolver(resolver);

		// JsonGraphFactory.instance.addJsonWriterAdapter(new RedpillRichTextJsonAdapter());
		// this.registerInfoProvider("bacongraph", "server", new ServerProvider());
		// this.registerInfoProvider("bacongraph", "user", new UserProvider());
